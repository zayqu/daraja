...
function checkRateLimit(request, { scope, limit, windowMs, rateIdentity }) {
  const identity = typeof rateIdentity === "string" && rateIdentity
    ? `actor:${rateIdentity.slice(0, 128)}`
...
export function protectMutation(
  ...
  if (!origin && requireOrigin) {
    return jsonError("Request origin required.", 403);
  }
  ...
  return checkRateLimit(request, { scope, limit, windowMs, rateIdentity });
}
