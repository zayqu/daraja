...
function isValidPastTimestamp(value, now) {
  ...
  return (
    ...
    now.getTime() - timestamp <= MAX_EVIDENCE_AGE_MS
  );
}

function isSafeAccessToken(value) {
  return (
    typeof value === "string" &&
    value.length >= 20 &&
    value.length <= 4_096 &&
...
function inspectWhatsAppSandboxConfiguration(env = process.env, now = new Date()) {
  const reasons = [];
  const enabled = env.WHATSAPP_SANDBOX_TRANSPORT_ENABLED === "true";
  const sandboxVerified = env.WHATSAPP_SANDBOX_VERIFIED === "true";
  const webhookVerified = env.WHATSAPP_SANDBOX_WEBHOOK_VERIFIED === "true";

  if (!enabled) reasons.push("WHATSAPP_SANDBOX_TRANSPORT_ENABLED must be true");
  if (!sandboxVerified) reasons.push("WHATSAPP_SANDBOX_VERIFIED must be true");
  if (!webhookVerified) {
    reasons.push("WHATSAPP_SANDBOX_WEBHOOK_VERIFIED must be true");
  }
  if (!isSafeEvidenceId(env.WHATSAPP_SANDBOX_EVIDENCE_ID)) {
    reasons.push("WHATSAPP_SANDBOX_EVIDENCE_ID is missing or invalid");
  }
  if (!isSafeEvidenceId(env.WHATSAPP_SANDBOX_WEBHOOK_EVIDENCE_ID)) {
    reasons.push("WHATSAPP_SANDBOX_WEBHOOK_EVIDENCE_ID is missing or invalid");
  }
  if (!isValidPastTimestamp(env.WHATSAPP_SANDBOX_VERIFIED_AT, now)) {
    reasons.push(
      "WHATSAPP_SANDBOX_VERIFIED_AT must be a canonical timestamp from the last 30 days",
    );
  }
  ...
  if (!isSafeAccessToken(env.WHATSAPP_ACCESS_TOKEN)) {
    reasons.push("WHATSAPP_ACCESS_TOKEN is missing or invalid");
  }
  ...
  return {
    ...
    evidenceId: isSafeEvidenceId(env.WHATSAPP_SANDBOX_EVIDENCE_ID)
      ...
      : null,
    webhookEvidenceId: isSafeEvidenceId(env.WHATSAPP_SANDBOX_WEBHOOK_EVIDENCE_ID)
      ? env.WHATSAPP_SANDBOX_WEBHOOK_EVIDENCE_ID
      : null,
...
function requireTemplateParameters(parameters) {
  ...
  return parameters.map((value) => {
    ...
    if (
      !text ||
      text.length > MAX_PARAMETER_LENGTH ||
      /[\u0000-\u001f\u007f]/.test(text)
    ) {
      throw new WhatsAppSandboxError(
...
async function sendWhatsAppSandboxTemplate({
  ...
  if (
    ...
    !/^wamid\.[A-Za-z0-9._~:+/=-]{1,480}$/.test(messageId)
  ) {
    throw new WhatsAppSandboxError(
  ...
  return {
    ...
    webhookEvidenceId: inspection.webhookEvidenceId,
    sandbox: true,
  };
}
